%GUI改制
clear;close all
selpath='C:\工程文件\ORCA\1567669452.100';
...
delimiter = ';';
formatSpec = '%f%f%f%f%f%*s%f%f%f%[^\n\r]';
fileID = fopen(filename,'r');
dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter, 'TextType', 'string', 'EmptyValue', NaN,  'ReturnOnError', false);
fclose(fileID);
data.imuData = table(dataArray{1:end-1}, 'VariableNames', {'yaw','pitch','roll','timenum','gpsYaw','lat','lng','mode'});
clearvars filename delimiter formatSpec fileID dataArray ans;
data.imuData.lat=data.imuData.lat/1;
data.imuData.lng=data.imuData.lng/1;
data.imuData.timenum=(data.imuData.timenum-data.imuData.timenum(1))/1.e6;%imu时间戳,单位秒
disp('imu数据加载完毕!');
...
figure;plot(data.gpsData.xEast,data.gpsData.yNorth)

figure;plot(data.TimeNum.imu,[data.imuData.yaw,data.imuData.roll,data.imuData.pitch]);legend('yaw','roll','pitch')

figure;plot(data.LidarData.reflectivity_abs)
% tangent_angle=atan2d(diff(data.gpsData.xEast),diff(data.gpsData.yNorth));
% tangent_angle=(tangent_angle<0)*360+tangent_angle;
% tangent_angle=[tangent_angle;tangent_angle(end)];
% tangent_angle=smooth(tangent_angle);
% tangent_angle=data.imuData.yaw(1)-tangent_angle(1)+tangent_angle;
% tangent_angle=-tangent_angle;
% tangent_angle=-(tangent_angle>180)*360+tangent_angle;
% figure;plot(tangent_angle);%hold on;plot(data.imuData.yaw);